...
export async function IgcAnalyze(trackFixes) { 
    if (trackFixes && trackFixes.length > 0) {
        ...
        try {
            ...
            console.error('IGC Analyze failed: ', error.message);
            return { success: false, message: error.message };
        }
    } else {
        return { success: false, message: 'Problem with trackFixes.fixes' };         
    }   
}
 ...
 class IGCAnalyzer {
    ...
    compute(fixes) { 
        ...
        try {
            ...
            console.log('Error in IGCAnalyzer.compute:', error);
            return { success: false, message: error.message };
        }
